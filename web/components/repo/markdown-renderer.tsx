...
function cleanupMermaidErrors(diagramId: string) {
  ...
  // Clean up d+id format elements mermaid may create
  const dElement = document.getElementById(`d${diagramId}`);
  if (dElement) {
    dElement.remove();
  }
  
  // Clean up mermaid error elements from body children
  document.querySelectorAll('body > div[id^="mermaid-"], body > div[id^="d"], body > svg[id^="mermaid-"]').forEach((el) => {
    // Check if element is a mermaid error (usually contains error text or is empty svg)
  ...
  useEffect(() => {
    ...
    const renderDiagram = async () => {
      ...
      try {
        ...
        // Clean up possible remaining error elements regardless of success/failure
...
// Pre-extract all heading text from markdown for unique id generation
