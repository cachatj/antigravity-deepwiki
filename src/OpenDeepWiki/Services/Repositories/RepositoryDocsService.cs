...
public class RepositoryDocsService(IContext context, IGitPlatformService gitPlatformService, ICache cache)
{
    ...
    private const int MaxConcurrentExports = 10; // Max concurrent exports
    ...
    public async Task<RepositoryBranchesResponse> GetBranchesAsync(string owner, string repo)
    {
        ...
        {
            // Get languages with actual document content for this branch
            ...
            // Only add branch if it has content
            if (languagesWithContent.Count > 0)
            {
                ...
                foreach (var lang in languagesWithContent)
                {
                    ...
                    // Record language marked as default
        ...
        // Determine default branch (only from branches with content)
        ...
        // Determine default language: prefer marked default, otherwise fallback
    ...
    public async Task<RepositoryTreeResponse> GetTreeAsync(string owner, string repo, [FromQuery] string? branch = null, [FromQuery] string? lang = null)
    {
        ...
        // Repository processing complete, get document catalog
        ...
        if (catalogs.Count == 0)
        {
            // Repository is complete but has no documents, possibly an empty repo
            return new RepositoryTreeResponse
            {
                Owner = repository.OrgName,
                Repo = repository.RepoName,
        ...
        // Recursively find first document with actual content
    ...
    private async Task<RepositoryBranch?> GetBranchAsync(string repositoryId, string? branchName)
    {
        ...
        // If branch name is specified, try to find it
        ...
        // Otherwise return default branch
    ...
    private async Task<BranchLanguage?> GetLanguageAsync(string branchId, string? languageCode)
    {
        ...
        // If language code is specified, try to find it
        ...
        // Prefer language marked as default
        ...
        // Fallback: use pre-set fallback language code
    ...
    private static string? FindFirstContentSlug(List<DocCatalog> catalogs, string? parentId)
    {
        ...
        {
            // If current node has content, return its path
