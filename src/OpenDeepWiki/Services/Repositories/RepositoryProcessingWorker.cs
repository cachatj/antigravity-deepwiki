    ...
    IOptions<WikiGeneratorOptions> wikiOptions) : BackgroundService
{
            ...
            try
            {
                ...
                if (processingLogService != null)
                {
                    await processingLogService.LogAsync(repository.Id, ProcessingStep.Complete, 
                        $"Repository processing completed, total time: {stopwatch.ElapsedMilliseconds}ms", cancellationToken: stoppingToken);
                }
            }
            catch (OperationCanceledException) when (stoppingToken.IsCancellationRequested)
            {
        ...
        CancellationToken stoppingToken)
    {
        ...
        {
            await processingLogService.LogAsync(repository.Id, ProcessingStep.Workspace, 
                $"Workspace preparation complete, Commit: {workspace.CommitId[..Math.Min(7, workspace.CommitId.Length)]}", cancellationToken: stoppingToken);
        }

        // Detect and update repository primary programming language (only on first processing or if empty)
        ...
        CancellationToken stoppingToken)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                // Mind map generated by MindMapWorker background task
                // Translation tasks handled by TranslationWorker background task
