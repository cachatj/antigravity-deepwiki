  ...
  useEffect(() => {
    ...
    const renderChart = async () => {
      ...
      try {
        ...
        // Preprocess chart to fix common syntax issues from AI-generated content.
        // Parentheses inside [...] or {...} node labels confuse the Mermaid parser
        // (e.g., `A[Staged (Silver) Layer]` — `(Silver)` is parsed as a shape).
        // Fix: wrap such labels in double quotes so Mermaid treats them as literal text.
        const preprocessChart = (raw: string): string => {
          return raw.replace(
            // Match node labels inside [...] that contain unquoted parentheses
            /\[([^\]"]*\([^\]]*)\]/g,
            (_match, content) => `["${content}"]`
          );
        };

        const sanitizedChart = preprocessChart(chart);
    ...
    return (
      <div className={`border border-[var(--highlight)]/30 rounded-md p-4 bg-[var(--highlight)]/5 ${className}`}>
        ...
        <div className="mt-3 text-xs text-[var(--muted)] font-serif">
          The chart contains a syntax error and cannot be rendered.
        </div>
      </div>
    );
  }
